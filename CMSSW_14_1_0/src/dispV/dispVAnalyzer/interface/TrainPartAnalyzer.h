#ifndef TrainPartAnalyzer_h
#define TrainPartAnalyzer_h
...
class TrainPartAnalyzer : public edm::stream::EDAnalyzer<edm::GlobalCache<ONNXRuntime>> {
   public:
      explicit TrainPartAnalyzer (const edm::ParameterSet&, const ONNXRuntime *);
      ~TrainPartAnalyzer();
   ...
   private:
      ...
      bool writeGNNFormat_;
      bool writeParTFormat_;
      bool sortTracksForParT_;
      ...
      std::vector<int> trk_label;
      std::vector<int> trk_hadidx;
      std::vector<int> trk_flav;
      std::vector<float> trk_delr;
      std::vector<float> trk_ptrat;
	
      std::vector<int> trk_1;
      std::vector<int> trk_2;
      std::vector<float> deltaR;
      std::vector<float> dca;
      std::vector<float> dca_sig;
      std::vector<float> cptopv;
      std::vector<float> pvtoPCA_1;
      std::vector<float> pvtoPCA_2;
      std::vector<float> dotprod_1;
      std::vector<float> dotprod_2;
      std::vector<float> edge_label;
      std::vector<int> truth_has_sv;
      std::vector<int> truth_has_b;
      std::vector<int> truth_has_btoc;
      std::vector<int> truth_has_c;

      std::vector<float> pair_dca;
      std::vector<float> pair_deltaR;
      std::vector<float> pair_dca_sig;
      std::vector<float> pair_cptopv;
      std::vector<float> pair_pvtoPCA_i;
      std::vector<float> pair_pvtoPCA_j;
      std::vector<float> pair_dotprod_i;
      std::vector<float> pair_dotprod_j;
      std::vector<float> pair_mom;
      std::vector<float> pair_invmass;
      std::vector<unsigned char> pair_label;
