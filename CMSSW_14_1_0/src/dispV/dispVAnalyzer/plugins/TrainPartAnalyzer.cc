// -*- C++ -*-
//
// Package:    Demo/TrainPartAnalyzer
// Class:      TrainPartAnalyzer
//
/**\class TrainPartAnalyzer TrainPartAnalyzer.cc Demo/TrainPartAnalyzer/plugins/TrainPartAnalyzer.cc
...
#include "dispV/dispVAnalyzer/interface/TrainPartAnalyzer.h"
...
// Constructor
TrainPartAnalyzer::TrainPartAnalyzer(const edm::ParameterSet& iConfig, const ONNXRuntime *cache):
	...
	writeGNNFormat_(iConfig.getUntrackedParameter<bool>("writeGNNFormat", true)),
	writeParTFormat_(iConfig.getUntrackedParameter<bool>("writeParTFormat", false)),
	sortTracksForParT_(iConfig.getUntrackedParameter<bool>("sortTracksForParT", false)),
	maxTracks_(iConfig.getUntrackedParameter<unsigned int>("maxTracks", 999999))
{
...
// Destructor
TrainPartAnalyzer::~TrainPartAnalyzer() {}


// Tell ONNXRunTime where is file location
std::unique_ptr<ONNXRuntime> TrainPartAnalyzer::initializeGlobalCache(const edm::ParameterSet &iConfig) 
{
    return std::make_unique<ONNXRuntime>(iConfig.getParameter<edm::FileInPath>("model_path").fullPath());
}

void TrainPartAnalyzer::globalEndJob(const ONNXRuntime *cache) {}


int TrainPartAnalyzer::checkPDG(int abs_pdg)
// Returns:
// 1 B hadron
// 2 D hadron
// 3 S hadron
...
int TrainPartAnalyzer::getDaughterLabel(const reco::GenParticle* dau)
{
    // Label meaning:
...
bool TrainPartAnalyzer::isGoodVtx(TransientVertex& tVTX){
...
std::vector<TransientVertex> TrainPartAnalyzer::TrackVertexRefit(std::vector<reco::TransientTrack> &Tracks,
...
void TrainPartAnalyzer::vertexMerge(std::vector<TransientVertex>& VTXs, double maxFraction, double minSignificance) 
...
TrainPartAnalyzer::TrackVertexArbitrator(const reco::Vertex& pv,
...
inline float TrainPartAnalyzer::sigmoid(float x) {
...
void TrainPartAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  using namespace edm;
  ...
  pair_dca_sig.clear();
  pair_cptopv.clear();
  pair_pvtoPCA_i.clear();
  pair_pvtoPCA_j.clear();
  pair_dotprod_i.clear();
  pair_dotprod_j.clear();
  pair_mom.clear();
  pair_invmass.clear();
  pair_label.clear();
  nPairs = 0;
  ...
  for (auto const& itrack : *patcan){
   ...
   // ------------------------------------------------------------
   // Optional ParT track sorting/truncation.
   // This changes the training sequence order. Pair arrays are then
   // written in deterministic upper-triangle order in this final order.
   // ------------------------------------------------------------
   std::vector<size_t> selectedTrackIdx;
   selectedTrackIdx.reserve(alltracks.size());
   
   for (size_t i = 0; i < alltracks.size(); ++i) {
       selectedTrackIdx.push_back(i);
   }
   
   if (writeParTFormat_ && sortTracksForParT_) {
       std::sort(
           selectedTrackIdx.begin(),
           selectedTrackIdx.end(),
           [&](size_t a, size_t b) {
               const auto tta = theB->build(alltracks[a]);
               const auto ttb = theB->build(alltracks[b]);
   
               float siga = 0.0f;
               float sigb = 0.0f;
   
               if (tta.isValid()) {
                   auto ip3d_a = IPTools::signedImpactParameter3D(tta, direction, pv).second;
                   siga = std::abs(ip3d_a.significance());
               }
   
               if (ttb.isValid()) {
                   auto ip3d_b = IPTools::signedImpactParameter3D(ttb, direction, pv).second;
                   sigb = std::abs(ip3d_b.significance());
               }
   
               if (siga != sigb) return siga > sigb;
               return alltracks[a].pt() > alltracks[b].pt();
           }
       );
   }
   
   if (writeParTFormat_ && maxTracks_ > 0 && selectedTrackIdx.size() > maxTracks_) {
       selectedTrackIdx.resize(maxTracks_);
   }
   
   std::vector<reco::Track> selectedTracks;
   selectedTracks.reserve(selectedTrackIdx.size());
   
   for (size_t idx : selectedTrackIdx) {
       selectedTracks.push_back(alltracks[idx]);
   }
   
   // From here onward, use selectedTracks for training output.
   const size_t num_tracks = selectedTracks.size();
   ...
   for (size_t i = 0; i< num_tracks; ++i) {
        t_trks[i] = (*theB).build(selectedTracks[i]);
	...
	double ip_z = t_trks[i].track().dz(pv.position());
        ...
        trk_p.push_back(selectedTracks[i].p());
        trk_pt.push_back(selectedTracks[i].pt());
        trk_eta.push_back(selectedTracks[i].eta());
        trk_phi.push_back(selectedTracks[i].phi());
        trk_charge.push_back(selectedTracks[i].charge());
        trk_nValid.push_back(selectedTracks[i].numberOfValidHits());
        trk_nValidPixel.push_back(selectedTracks[i].hitPattern().numberOfValidPixelHits());
        trk_nValidStrip.push_back(selectedTracks[i].hitPattern().numberOfValidStripHits());
        ntrk++;
   	...
   	// Parallelize the outer loop
   	 ...
   	 for (size_t i = 0; i < num_tracks; ++i) {
   	     ...
   	     for (size_t j = i + 1; j < num_tracks; ++j) {
   	         ...
   	         float eta1 = selectedTracks[i].eta();
   	         float phi1 = selectedTracks[i].phi();
   	         float eta2 = selectedTracks[j].eta();
   	         float phi2 = selectedTracks[j].phi();
   	         ...
   	         float px1 = selectedTracks[i].px();
   	         float py1 = selectedTracks[i].py();
   	         float pz1 = selectedTracks[i].pz();
   	         
   	         float px2 = selectedTracks[j].px();
   	         float py2 = selectedTracks[j].py();
   	         float pz2 = selectedTracks[j].pz();
   	     ...
   	     for (size_t j = i + 1; j < num_tracks; ++j) {
   	         ...
   	         if (minDist.calculate(t_trks[i].impactPointState(), t_trks[j].impactPointState())) {
   	          ...
   	          //if (pairMomentumMag < 0.05 or pairMomentumMag > 100) continue;
   	     		...
   	     		pair_deltaR.push_back(delta_r_val);
   	     		pair_dca.push_back(dca_val);
   	     		pair_dca_sig.push_back(dcaSig_val);
   	     		pair_cptopv.push_back(cptopv_val);
   	     		pair_pvtoPCA_i.push_back(pvToPCAseed_val);
   	     		pair_pvtoPCA_j.push_back(pvToPCAtrack_val);
   	     		pair_dotprod_i.push_back(dotprodSeed_val);
   	     		pair_dotprod_j.push_back(dotprodTrack_val);
   	     		pair_mom.push_back(pairMomentumMag);
   	     		pair_invmass.push_back(inv_mass);
   	     }
   	 }
   ...
   // ------------------------------------------------------------
   // Pair labels.
   // GNN mode: label explicit trk_1/trk_2 edges.
   // ParT mode: label deterministic upper-triangle pairs.
   // ------------------------------------------------------------
   if (writeGNNFormat_) {
   ...
   if (writeParTFormat_) {
       const size_t n = trk_hadidx.size();
       const size_t expectedPairs = n * (n - 1) / 2;
   
       pair_label.reserve(expectedPairs);
   
       for (size_t i = 0; i < n; ++i) {
           for (size_t j = i + 1; j < n; ++j) {
               const bool sameHadron =
                   trk_hadidx[i] >= 0 &&
                   trk_hadidx[i] == trk_hadidx[j] &&
                   trk_flav[i] == trk_flav[j];
   
               pair_label.push_back(sameHadron ? 1 : 0);
           }
       }
   
       nPairs = static_cast<unsigned int>(pair_label.size());
   
       if (pair_dca.size() != pair_label.size()) {
           edm::LogWarning("TrainAnalyzer")
               << "ParT pair feature/label size mismatch: "
               << "pair_dca=" << pair_dca.size()
               << " pair_label=" << pair_label.size()
               << " nTracks=" << n;
       }
   } 
...
void TrainPartAnalyzer::beginStream(edm::StreamID) {
	...
	if (writeGNNFormat_) { 
        ...
        if (writeParTFormat_) {
	        tree->Branch("nPairs", &nPairs, "nPairs/i");
	    
	        tree->Branch("pair_deltaR", &pair_deltaR);
	        tree->Branch("pair_dca", &pair_dca);
	        tree->Branch("pair_dca_sig", &pair_dca_sig);
	        tree->Branch("pair_cptopv", &pair_cptopv);
	        tree->Branch("pair_pvtoPCA_i", &pair_pvtoPCA_i);
	        tree->Branch("pair_pvtoPCA_j", &pair_pvtoPCA_j);
	        tree->Branch("pair_dotprod_i", &pair_dotprod_i);
	        tree->Branch("pair_dotprod_j", &pair_dotprod_j);
	        tree->Branch("pair_mom", &pair_mom);
	        tree->Branch("pair_invmass", &pair_invmass);
	        tree->Branch("pair_label", &pair_label);
	}
...
//void TrainPartAnalyzer::endJob() {
//}

// ------------ method fills 'descriptions' with the allowed parameters for the module  ------------
void TrainPartAnalyzer::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
DEFINE_FWK_MODULE(TrainPartAnalyzer);
