...
class EdgeMLPConv(nn.Module):
    ...
    def forward(self, x, edge_index, edge_attr):
        ...
        x_i = x[tgt]  # target nodes
        x_j = x[src]  # source nodes

        msg = self.mlp(torch.cat([x_i, x_j, edge_attr], dim=1))

        if torch.onnx.is_in_onnx_export():
            print("Export one hot EdgeMLP")
            num_nodes = x.size(0)
            identity = torch.eye(num_nodes, device=x.device)
            one_hot = identity[tgt]  # Replaces F.one_hot
            agg = one_hot.T @ msg
            deg = one_hot.T @ torch.ones_like(msg[:, :1])
            return agg / deg.clamp(min=1.0)
        else:
            return scatter_add(msg, tgt, dim=0, dim_size=x.size(0)) / \
                   scatter_add(torch.ones_like(msg[:, :1]), tgt, dim=0, dim_size=x.size(0)).clamp(min=1.0)
...
class GNNModel(torch.nn.Module):
    ...
    def forward(self, x_in, edge_index, edge_attr):
        ...
        if torch.onnx.is_in_onnx_export():
            ...
            num_edges = e_idx[1].size(0)
            identity = torch.eye(num_nodes, device=x.device)
            one_hot = identity[e_idx[1]]  # same shape as F.one_hot
